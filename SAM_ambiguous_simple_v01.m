...
try
    ...
    % Wait 1 sec before starting showing the dots
    WaitSecs(1)
    ...
    while currentTime < endTime
        ...
        if ismember(currentFr, changeFr)
            ...
            if portTrigg
                % send trigger with the new position
                if syncbox==1
                    lptwrite(PortAddress, currstate);
                    WaitSecs(0.004);
                    lptwrite(PortAddress, 0);
                elseif syncbox==2
                    io64(ioObj, PortAddress, currstate);
                    WaitSecs(0.004);
                    io64(ioObj, PortAddress, 0);
                end
            end
        ...
        % % % %        % Get screenshot
        % % % %         ImageArrayN=Screen('GetImage', windowID, [], [], [], []);
        ...
        if keyIsDown == 1
            ...
            if iscell(keystring)
                ...
                if button1 == 0 && button2 == 0
                    ...
                    if portTrigg
                        if syncbox==1
                            lptwrite(PortAddress, 222);
                            WaitSecs(0.004);
                            lptwrite(PortAddress, 0);
                        elseif syncbox==2
                            io64(ioObj, PortAddress, 222);
                            WaitSecs(0.004);
                            io64(ioObj, PortAddress, 0);
                        end
                    end
