...
% 1 to send state (0 or 1 triggers)
sendStateTrigger = 0;
...
if configs.portTrigg
    if configs.syncbox == 1
        addpath('libs/ParallelPortLibraries/');
        configs.PortAddress = hex2dec('E800'); %-configure paralell port adress
    elseif configs.syncbox == 2
        addpath('libs/ParallelPortLibraries/');
        configs.ioObj = io64;
        status = io64(configs.ioObj);
        configs.PortAddress = hex2dec('378'); %-configure paralell port adress
        data_out=1;
        io64(configs.ioObj, configs.PortAddress, data_out);
        data_out=0;
        pause(0.01);
        io64(configs.ioObj, configs.PortAddress, data_out);
    end
...
try
    ...
    % Send trigger.
    success=sendTrigger(configs, trigReady); % trigger 35 - start?
    ...
    % Send trigger.
    success=sendTrigger(configs, trigBaseline); % trigger 35 - start?
    ...
    while currentTime < endTime
        ...
        if ismember(currentFr, changeFr)
            ...
            if sendStateTrigger
                % Send trigger.
                success=sendTrigger(configs, currstate); % send trigger with the new position
            end
        ...
        if keyIsDown == 1
            ...
            if iscell(keystring)
                ...
                if keyCode(escapekeycode)
                    ...
                    % Send trigger.
                    success=sendTrigger(configs, trigEnd);
                ...
                if button1 == 0 && button2 == 0
                    ...
                    % Send trigger.
                    success=sendTrigger(configs, trigDouble);
                ...
                elseif keyCode(horzkeycode) && button1 == 1 && button2 == 0
                    ...
                    % Send trigger.
                    success=sendTrigger(configs, trigInward);
                ...
                elseif keyCode(vertkeycode) && button1 == 0 && button2 == 1
                    ...
                    % Send trigger.
                    success=sendTrigger(configs, trigDownward);
                ...
                elseif keyCode(horzkeycode) && button1 == 1 && button2 == 1  && doublepress < 40
                    ...
                    success=sendTrigger(configs, trigStopDownward);
                ...
                elseif keyCode(vertkeycode) && button1 == 1 && button2 == 1  && doublepress < 40
                    ...
                    success=sendTrigger(configs, trigStopInward);
                ...
                elseif keyCode(horzkeycode) && button1 == 1 && button2 == 1  && doublepress > 40
                    ...
                    success=sendTrigger(configs, trigStopDouble);
                ...
                elseif keyCode(vertkeycode) && button1 == 1 && button2 == 1  && doublepress > 40
                    ...
                    % Send trigger for StopDouble.
                    ssuccess=sendTrigger(configs, trigStopDouble);
